...
def notify_success(context):
    ...
    log.info(f"[SUCCESS] Tarea completada: {context['task_instance'].task_id}")

def notify_failure(context):
    log = logging.getLogger("airflow.task")
    log.error(f"[FAILURE] DAG falló: {context['dag'].dag_id}")
    log.error(f"Tarea con error: {context['task_instance'].task_id}")
    log.error(f"Error: {context.get('exception')}")
...
def etl_pipeline():
    ...
    fallo = fallo_intencional()

    end = end_dag()

    start >> consultar_params >> [test_xcom, transform] >> fallo >> end
